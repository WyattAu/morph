...
and access control for the Morph language. It provides mathematical definitions
...
abbrev Env := List (String × TypeWithVisibility)

/-- Check if symbol is in environment -/
def Env.contains (env : Env) (sym : String) : Bool :=
  env.any fun (s, _) => s = sym

/-- Get type of symbol from environment -/
def Env.getType (env : Env) (sym : String) : Option TypeWithVisibility :=
...
These specifications define the correctness properties of the module system.
-/
...
def spec_module_privacy (mod : ModuleDecl) (env : Env) : Prop :=
  IsPrivateModule mod →
  ∀ (sym : String), sym ∈ mod.exports →
    env.contains sym ∧
...
def spec_module_interface (mod : ModuleDecl) (iface : ModuleInterface) (env : Env) : Prop :=
...
def ModuleDecl.isEncapsulated (mod : ModuleDecl) (env : Env) : Prop :=
  IsPrivateModule mod →
  ∀ (sym : String), sym ∉ mod.exports →
    (¬env.contains sym ∨
...
def spec_module_encapsulation (mod : ModuleDecl) (impl : ModuleImplementation) (env : Env) : Prop :=
  mod.hasImplementation impl ∧
...
def spec_module_access_control (mod : ModuleDecl) (acl : AccessControl) (env : Env) : Prop :=
  IsPrivateModule mod →
...
def spec_module_composition (comp : ModuleComposition) (env : Env) : Prop :=
