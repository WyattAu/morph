...
import Morph.Specs.Ab iDataRefinement.Spec
...
-!
namespace Morph.Specs.Ab iDataRefinement
...
def example_validate_layout : Bool :=
  validateLayout { abiType := { name := "i32", size := 32, align := 32 }, [0]
...
def example_compatible_layouts : Bool :=
  compatibleLayouts
    { abiType := { name := "i32", size := 32, align := 32 }, [0] }
    { abiType := { name := "i32", size := 32, align := 32 }, [0] }
...
end Morph.Specs.Ab iDataRefinement
