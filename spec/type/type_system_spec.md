...
This specification defines the Type System of Morph, providing formal foundation for type safety, memory management, and effect tracking. The type system combines affine logic, capability-based ownership, and static effect typing to ensure memory safety without garbage collection while enabling high-performance concurrency.
...
- [`spec/type/pure_type_spec.md`](type/pure_type_spec.md) - Formal definition of Pure type and its role in the type system
...
where:
- `GenericTypeName`: Name of the generic type or function
...
- TYP-INV-015:* THE system SHALL validate generic type syntax according to the rules above.
...
where $\sqsubseteq$ denotes the effect subtyping relation (less effects ⊆ more effects).
...
4. **Let Binding:* If $\Gamma \vdash e_1 : T_1$ and $\Gamma, x:T_1 \vdash e_2 : T_2$, then $\Gamma \vdash \text{let } x = e_1 \text{ in } e_2 : T_2$ and $\text{safe}(\text{let } x = e_1 \text{ in } e_2)$.
   ...
   - Variable $x$ is bound to this value with type $T_1$
   - Extended environment $\Gamma, x:T_1$ is well-typed
...
5. **Pattern Matching:* If $\Gamma \vdash e : T$ and for each variant $V_i$ of sum type $T$, $\Gamma, x_i:T_i \vdash e_i : T'$ where $T_i$ is the type of variant $V_i$, then $\Gamma \vdash \text{fix } e \{ V_i(x_i) \Rightarrow e_i \} : T'$ and $\text{safe}(\text{fix } e \{ V_i(x_i) \Rightarrow e_i \})$.
   ...
   - Corresponding branch $e_i$ is executed with correctly typed variable $x_i$
   - All branches return type $T'$
   - Therefore, pattern matching is type-safe

6. **Capability Operations:* For all capability transitions, if the source expression is type-safe, then the result is type-safe.
   ...
   **6.4.2 Borrow Operation (Iso/Val → Ref):*
   ...
   - **Proof:*
     ...
     4. The type system ensures the reference cannot outlive the source
     5. Therefore, borrow preserves type safety
...
10. **Comprehensive Type Safety:*
   ...
   - **Proof:*
     ...
     2. All language constructs are covered by the induction
...
let p2: Point = p1;  // Value semantics: copies the struct
```
