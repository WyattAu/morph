...
1.  **Topology:* Is "Submit" inside of "Form"?
2.  **Geometry:* Is "Cancel" button overlapping with "OK" button?
3.  **State:* Did clicking "Login" actually change its screen to Dashboard?
...
- **Process:*
  ...
  2.  Runtime builds an Accessibility Tree.
...
- **Purpose:* Allows Agent to run End-to-End (E2E) tests on its own UI.
- **Actions:*
  - `tap`: Simulates a click/touch at the center of node's `rect`.
...
The Agent is trained to perform "Spatial Unit Tests" using SAP data.
...
- **Scenario:*
  1.  `set_viewport(1920, 1080)` → `get_ui_snapshot()` → Verify Sidebar is visible.
  2.  `set_viewport(400, 800)` → `get_ui_snapshot()` → Verify Sidebar is hidden (Hamburger menu).
